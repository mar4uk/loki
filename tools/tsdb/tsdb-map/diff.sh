...
LOKI_TSDB_PATH=/tmp/loki-tsdb-a go test github.com/mar4uk/loki/tools/tsdb/tsdb-map -count="${COUNT}" -bench=BenchmarkQuery -run '^$' -benchmem > /tmp/loki-tsdb-bench-a
...
LOKI_TSDB_PATH=/tmp/loki-tsdb-b go test github.com/mar4uk/loki/tools/tsdb/tsdb-map -count="${COUNT}" -bench=BenchmarkQuery -run '^$' -benchmem > /tmp/loki-tsdb-bench-b
