...
* Deploy the [Loki Operator](https://github.com/mar4uk/loki/blob/master/operator/docs/hack_loki_operator.md#hacking-on-loki-operator-on-openshift) to the cluster.
...
* Fork and clone the [Loki Operator](https://github.com/mar4uk/loki/blobs/master/operator) repo.

* All the files related to storage size calculator exists at [`config/overlays/openshift/size-calculator`](https://github.com/mar4uk/loki/tree/master/operator/config/overlays/openshift/size-calculator) and the main file is at [`cmd/size-calculator`](https://github.com/mar4uk/loki/tree/master/operator/cmd/size-calculator).

* Update the code to fix a bug or add a new feature.

* To test the changes made, build the image and push it to quay. Replace [here](https://github.com/mar4uk/loki/blob/master/operator/config/overlays/openshift/size-calculator/storage_size_calculator.yaml#L18) with your quay image to test the changes.
  ...
  This will build the storage size calculator image using [dockerfile](https://github.com/mar4uk/loki/blob/master/operator/calculator.Dockerfile)
...
If you get `permission denied` error while running `make deploy-size-calculator` then make [this](https://github.com/mar4uk/loki/blob/master/operator/hack/deploy-prometheus-secret.sh) file executable by running:
